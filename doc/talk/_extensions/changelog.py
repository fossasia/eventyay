...
def release_role(name, rawtext, text, lineno, inliner, *args, **kwargs):
    ...
    text = number
    url = f"https://pypi.org/project/Eventyay/{number.strip('v')}/"
    ...
    releases = [
        {
            "release": _build_release_node(
                "next",
                "https://github.com/fossasia/eventyay/commits/main/",
                text="Next Release",
            ),
...
def construct_issue_nodes(issue, description):
    ...
    if issue.number:
        ref = f"https://github.com/fossasia/eventyay/issues/{issue.number}"
