...
class RoomModule(BaseModule):
    ...
    async def config_patch(self, body):
        ...
        logger.info(f"[ROOM_UPDATE] room_id={self.room.id}, submitted: setup_complete={body.get('setup_complete')}, sidebar_hidden={body.get('sidebar_hidden')}, module_config_length={len(body.get('module_config', [])) if body.get('module_config') else 0}")
