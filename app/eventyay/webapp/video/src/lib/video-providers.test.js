...
test('dropdown labels match the organiser create options', () => {
	assert.deepEqual(
		...
		['Stream (YT, HLS)', 'BBB', 'Jitsi', 'Janus', 'LoungeMesh']
	)
})
...
test('enabled Jitsi, Janus, and LoungeMesh flags include those providers', () => {
	const providers = getAvailableVideoProviders(
		allow(['room:update']),
		false,
		features(['jitsi', 'janus', 'loungemesh'])
	)
	assert.deepEqual(providers.map(provider => provider.id), ['stream', 'bbb', 'jitsi', 'janus', 'loungemesh'])
})
...
test('configured room labels include the video provider', () => {
	...
	assert.equal(getConfiguredRoomLabel({id: 'channel-loungemesh', name: 'LoungeMesh'}), 'Video Channel: LoungeMesh')
...
test('LoungeMesh stays hidden when its feature flag is off', () => {
	assert.equal(
		isVideoProviderEnabled(
			VIDEO_CREATE_PROVIDERS.find(provider => provider.id === 'loungemesh'),
			features([])
		),
		false
	)
...
test('LoungeMesh create permission is enough without room:update', () => {
	const providers = getAvailableVideoProviders(
		allow(['world:rooms.create.loungemesh']),
		false,
		features(['loungemesh'])
	)
	assert.deepEqual(providers.map(provider => provider.id), ['loungemesh'])
})

test('applying LoungeMesh sets an empty features config', () => {
	const config = { module_config: [] }
	assert.equal(
		applyVideoProviderToConfig(config, {id: 'channel-loungemesh', startingModule: 'call.loungemesh'}),
		true
	)
	assert.deepEqual(config.module_config, [{
