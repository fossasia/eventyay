...
transition(name="sidebar")
	.c-rooms-sidebar(v-show="show && !snapBack", :style="style", role="navigation", @pointerdown="onPointerdown", @pointermove="onPointermove", @pointerup="onPointerup", @pointercancel="onPointercancel")
		scrollbars(y)
			...
			.group-title#chats-title(v-if="roomsByType.videoChat.length || roomsByType.textChat.length || hasPermission('world:rooms.create.chat') || hasPermission('world:rooms.create.bbb') || hasPermission('world:rooms.create.jitsi')")
				span {{ $t('RoomsSidebar:channels-headline:text') }}
				.buffer
				bunt-icon-button(v-if="hasPermission('world:rooms.create.chat') || hasPermission('world:rooms.create.bbb') || hasPermission('world:rooms.create.jitsi')", tooltip="Create Channel", :tooltip-fixed="true", @click="showChatCreationPrompt = true") plus
			...
			.chats(v-if="roomsByType.videoChat.length || roomsByType.textChat.length || hasPermission('world:rooms.create.chat') || hasPermission('world:rooms.create.bbb') || hasPermission('world:rooms.create.jitsi')", role="group", aria-describedby="chats-title")
