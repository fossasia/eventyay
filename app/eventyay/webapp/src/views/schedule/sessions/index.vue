...
export default {
	...
	async created () {
		...
		try {
			...
			console.error('[Sessions] Failed to load exporters:', error)
		}
	},
	watch: {
		tracksFilter: {
	...
	methods: {
		...
		handleDayChangeByScroll(luxonDay) {
			...
			if (this.$refs.tabs) {
				const tabEl = this.$refs.tabs.$refs.tabElements?.find(el => el.id === momentDay.toISOString())
				tabEl?.$el?.scrollIntoView()
			}
		},
		getTrackName(track) {
		...
		async makeExport() {
			try {
				...
				this.isExporting = false
				this.error = error
				console.error('[Sessions] Export failed:', error)
			}
		},
		resetAllFiltered() {
			this.resetFiltered()
