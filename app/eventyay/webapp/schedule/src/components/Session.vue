...
a.c-linear-schedule-session(:class="{faved, 'has-date': showDate, 'short-session': isShortSession, 'grid-very-short': isGridVeryShort, 'schedule-pending-session': isSchedulePending, 'has-fav-count': hasFavCount}", :style="style", :href="link", @click="onSessionLinkClick($event, session)", :target="linkTarget")
	...
	.info(:class="{'has-icons': hasAnyRightIcons, 'grid-session-info': showSessionType}")
		...
		.interpretation(v-if="showInterpretation", :class="{'with-do-not-record': session.do_not_record}", :title="interpretationTooltip", :aria-label="interpretationTooltip")
...
export default {
	...
	computed: {
		...
		showInterpretation () {
			if (this.session.has_interpretation) return true
		...
		hasAnyRightIcons () {
			return !this.favsReadOnly || this.canOpenStream || this.session.do_not_record || this.showInterpretation
		},
		isShortSession () {
		...
		shouldClampTitle () {
			if (this.showSessionType) {
				return this.isVeryShortSession
			}
			return this.isShortSession
		},
		isGridVeryShort () {
