    ...
    def clean(self, value, *args, **kwargs):
        ...
        if isinstance(value, UploadedFile):
            ...
            if process.returncode != 0:
                logger.info(f'Trying to convert a DER to PEM failed: {error}')
                raise ValidationError(
...
class PNGImageField(forms.FileField):
    ...
    def clean(self, value, *args, **kwargs):
        ...
        if isinstance(value, UploadedFile):
            ...
            value.seek(0)
            try:
                pillow_formats = getattr(settings, 'PILLOW_FORMATS_IMAGE', None)
                open_kwargs = {'formats': pillow_formats} if pillow_formats else {}
                with (
                    Image.open(value, **open_kwargs) as im,
                ...
                raise ValidationError(_('The file you uploaded could not be converted to PNG format.'))
