...
def clean_cached_tickets(sender, **kwargs):
    for cf in CachedTicket.objects.filter(created__lte=now() - settings.CACHE_TICKETS_MAX_AGE):
        cf.delete()
    for cf in CachedCombinedTicket.objects.filter(created__lte=now() - settings.CACHE_TICKETS_MAX_AGE):
        cf.delete()
