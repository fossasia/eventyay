...
def _event_entry(ev):
    ...
    start_utc = ev.date_from.astimezone(UTC)
    end_utc = (
        ev.date_to.astimezone(UTC) if event.settings.show_date_to and ev.date_to else start_utc + timedelta(hours=1)
    )
...
def build_organizer_frab_data(organizer, events, base_url):
    ...
    if not by_date:
        ...
        date_bounds[today] = {
            ...
            'end': datetime.combine(today, datetime.max.time()).replace(tzinfo=UTC),
        }
    ...
    conf_start = min(all_starts) if all_starts else datetime.now(UTC)
