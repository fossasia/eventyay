...
class TestCacheKeys:
    def test_schedule_json_stamp_key_is_consistent(self):
        key1 = schedule_json_stamp_key(42)
        key2 = schedule_json_stamp_key(42)
        assert key1 == key2
...
class TestBuildDataCacheHitMiss:
    ...
    def test_versioned_schedule_result_is_cached(self, schedule_with_version):
        ...
        with scope(event=sched.event):
            ...
            assert stamp == 0, 'No invalidation yet; stamp should be untouched'
    ...
    def test_invalidate_causes_cache_miss_on_next_call(self, schedule_with_version):
        sched = schedule_with_version
        cache.clear()
        with scope(event=sched.event):
            result1 = sched.build_data()
        ...
        with scope(event=sched.event):
            result2 = sched.build_data()
        assert result1 == result2, 'Content must be identical after re-computation'
