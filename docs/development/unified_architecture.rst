...
All components are now integrated into a single codebase under the ``src/eventyay`` directory, sharing a unified data model, authentication system, and user interface.
...
All components share a common Django-based data model located in ``src/eventyay/base/models/``. Key shared entities include:
...
All components utilize shared services from ``src/eventyay/base/services/``:
...
* ``src/eventyay/static/`` - Shared static assets
* ``webapps/`` - Frontend applications and shared component libraries
* ``webapps/video/`` - Video Vue 3 SPA
...
1. **Database Migration:** Use the provided migration scripts in ``src/eventyay/base/migrations/``
